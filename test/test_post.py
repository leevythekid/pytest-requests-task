...
class TestPostMethod(TestBase):
    ...
    def test_post_create_playlist_status_code(self, playlist_name, playlist_desc, is_playlist_public):
        ...
        ), "Status code is expected to be: 201."
    ...
    def test_post_create_playlist_properties(self, playlist_name, playlist_desc, is_playlist_public):
        ...
        assert (
            response.json()["name"] == playlist_name
        ), f"Name is expected to be: {playlist_name}"
        assert (
            response.json()["description"] == playlist_desc
        ), f"Description is expected to be: {playlist_desc}"
        assert (
            response.json()["public"] == is_playlist_public
        ), f"Visibility is expected to be: {is_playlist_public}"
    ...
    def test_post_add_items_to_playlist_status_code(self, create_playlist):
        ...
        assert (
            response.status_code == 201
        ), f"Response status code is expected to be: 201."
